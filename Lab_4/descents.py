...
class BaseDescent:
    ...
    def __init__(self, dimension: int, lambda_: float = 1e-3, loss_function: LossFunction = LossFunction.MSE):
        ...
        """
        self.w: np.ndarray = np.random.rand(dimension) * 0.001
...
class VanillaGradientDescent(BaseDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        if self.loss_function is LossFunction.MSE:
            ...
            gradient = np.dot(x.T, np.tanh(y_pred - y)) / len(y)
...
class StochasticDescent(VanillaGradientDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        if self.loss_function is LossFunction.MSE:
            # Усредненный градиент по мини-батчу
            gradient = 2 * np.dot(x_batch.T, (y_pred - y_batch)) / self.batch_size
        elif self.loss_function is LossFunction.LogCosh:
            gradient = np.dot(x_batch.T, np.tanh(y_pred - y_batch)) / self.batch_size
...
class Adam(VanillaGradientDescent):
    ...
    def update_weights(self, gradient: np.ndarray) -> np.ndarray:
        ...
        # Чуть ломается логика с формулой. На первой итерации будет 1, а не 0, иначе деление на 0
        self.iteration += 1

        m_sh = self.m / (1 - self.beta_1**self.iteration)
        v_sh = self.v / (1 - self.beta_2**self.iteration)

        diff_w = self.lr() * m_sh / (np.sqrt(v_sh) + self.eps)
